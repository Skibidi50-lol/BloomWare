...
-- ================================================
-- SILENT AIM - PURE Mouse.Hit (BEST FOR PRISON LIFE)
-- ================================================
...
-- Silent Aim Settings (Mouse.Hit Only)
getgenv().SilentAim = {
    Enabled          = false,
    TeamCheck        = true,
    VisibleCheck     = true,
    ...
    ShowFOV          = true,
    Prediction       = true,
    PredictionAmount = 0.135,   -- Perfect value for Prison Life guns
    HitChance        = 100,
    ShowTargetBox    = true
}
...
FOVCircle.Visible = getgenv().SilentAim.ShowFOV

local TargetIndicator = Drawing.new("Square")
TargetIndicator.Size = Vector2.new(22, 22)
TargetIndicator.Thickness = 2
TargetIndicator.Filled = false
TargetIndicator.Color = Color3.fromRGB(255, 0, 255)
TargetIndicator.Visible = false

-- Get Closest Player (FOV + Visibility)
local function GetClosestPlayer()
    local closest = nil
    local shortestDist = getgenv().SilentAim.FOV
    local mousePos = UserInputService:GetMouseLocation()

    for _, player in ipairs(Players:GetPlayers()) do
        if player == LocalPlayer then continue end
        if getgenv().SilentAim.TeamCheck and player.Team == LocalPlayer.Team then continue end

        local char = player.Character
        if not char or not char:FindFirstChild("HumanoidRootPart") then continue end
        if not char:FindFirstChild("Humanoid") or char.Humanoid.Health <= 0 then continue end

        local part = char:FindFirstChild(getgenv().SilentAim.TargetPart) or char.HumanoidRootPart
        local screenPos, onScreen = Camera:WorldToViewportPoint(part.Position)
...
-- Mouse.Hit / Mouse.Target Hook (PURE MOUSE.HIT)
local oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, key)
    if not getgenv().SilentAim.Enabled then return oldIndex(self, key) end
    if self ~= Mouse then return oldIndex(self, key) end
    if not (key == "Hit" or key == "Target") then return oldIndex(self, key) end
    ...
    elseif key == "Hit" then
        ...
        if getgenv().SilentAim.Prediction and target.Parent:FindFirstChild("HumanoidRootPart") then
            predictedPos = predictedPos + (target.Parent.HumanoidRootPart.Velocity * getgenv().SilentAim.PredictionAmount)
        end
...
RunService.RenderStepped:Connect(function()
    ...
    FOVCircle.Radius = getgenv().SilentAim.FOV
    FOVCircle.Visible = getgenv().SilentAim.Enabled and getgenv().SilentAim.ShowFOV

    if getgenv().SilentAim.Enabled and getgenv().SilentAim.ShowTargetBox then
        local target = GetClosestPlayer()
        if target then
            local screenPos = Camera:WorldToViewportPoint(target.Position)
            TargetIndicator.Position = Vector2.new(screenPos.X, screenPos.Y) - TargetIndicator.Size/2
...
local silentAimBox = Tabs.Main:AddRightGroupbox("Silent Aim", "crosshair")

silentAimBox:AddCheckbox("SA_Enabled", {
    Text = "Enable Silent Aim",
    Default = false,
    Tooltip = "Pure Mouse.Hit - Most Undetectable"
}):OnChanged(function(v)
    getgenv().SilentAim.Enabled = v
    Library:Notify({Title = "Silent Aim", Description = v and "ON (Mouse.Hit)" or "OFF", Time = 2})
end)

silentAimBox:AddCheckbox("SA_TeamCheck", {Text = "Team Check", Default = true})
    :OnChanged(function(v) getgenv().SilentAim.TeamCheck = v end)

silentAimBox:AddCheckbox("SA_VisibleCheck", {Text = "Wall Check", Default = true})
    :OnChanged(function(v) getgenv().SilentAim.VisibleCheck = v end)

silentAimBox:AddDropdown("SA_Part", {
    Values = {"Head", "HumanoidRootPart", "UpperTorso"},
    Default = "Head",
    Text = "Target Part"
}):OnChanged(function(v) getgenv().SilentAim.TargetPart = v end)

silentAimBox:AddSlider("SA_FOV", {
    Text = "FOV Radius",
    Min = 10, Max = 400, Default = 130, Rounding = 0
}):OnChanged(function(v) getgenv().SilentAim.FOV = v end)

silentAimBox:AddCheckbox("SA_ShowFOV", {Text = "Show FOV Circle", Default = true})
    :OnChanged(function(v) getgenv().SilentAim.ShowFOV = v end)

silentAimBox:AddCheckbox("SA_Prediction", {Text = "Prediction", Default = true})
    :OnChanged(function(v) getgenv().SilentAim.Prediction = v end)

silentAimBox:AddSlider("SA_PredAmt", {
    Text = "Prediction Amount",
    Min = 0.1, Max = 0.2, Default = 0.135, Rounding = 3
}):OnChanged(function(v) getgenv().SilentAim.PredictionAmount = v end)

silentAimBox:AddSlider("SA_HitChance", {
    Text = "Hit Chance %",
    Min = 50, Max = 100, Default = 100, Rounding = 0
}):OnChanged(function(v) getgenv().SilentAim.HitChance = v end)
